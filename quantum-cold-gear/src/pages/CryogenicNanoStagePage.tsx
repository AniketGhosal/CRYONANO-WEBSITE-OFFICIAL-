...
import { 
  Download, ArrowUp, CheckCircle2, ChevronRight, ChevronDown, Plus, Minus,
  Activity, Zap, Shield, Camera, Cpu, Target, Settings, ThermometerSnowflake,
  Waves, Layers, Network, Lock, ShieldAlert, Code, ArrowLeft, ArrowRight, Move, Wind, Eye, Image, Copy
} from "lucide-react";
import { Link } from "react-router-dom";
import { productsData } from "@/data/products";
...
// Icon mapping dictionary
const iconMap: Record<string, any> = { 
  ThermometerSnowflake, Activity, Zap, Shield, Camera, Move, Waves, Target, Cpu, Settings, Layers, Lock, ShieldAlert, Code, Wind, Network
};
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-background text-foreground antialiased selection:bg-primary selection:text-white relative">
        ...
        {/* 01. HERO SECTION */}
        {/* <section className="relative pt-2 pb-10 overflow-hidden bg-gradient-to-b from-[#061B35] to-[#0A2647] text-white"> */}
              ...
              {product.coreTechnologies.map((tech: any, i: number) => (
                <div key={i} className={`grid md:grid-cols-12 gap-10 items-center ${i % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
                  <motion.div 
                    ...
                    className={`md:col-span-6 ${i % 2 === 1 ? 'md:col-start-7 md:row-start-1' : 'md:col-start-1'}`}
                  >
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded bg-slate-100 text-[#0A2647] text-xs font-bold uppercase tracking-wider mb-4 border border-border shadow-sm">
                      {tech.type === 'hardware' ? <Waves className="w-4 h-4 text-primary" /> : tech.type === 'software' ? <Code className="w-4 h-4 text-primary" /> : <Lock className="w-4 h-4 text-primary" />} {tech.subtitle}
                    </div>
                    <h3 className="text-2xl font-extrabold text-[#0A2647] mb-4">{tech.title}</h3>
                    <p className="text-base text-slate-600 font-medium leading-relaxed mb-6">{tech.description}</p>
                    <ul className="space-y-3">
                      {tech.bullets.map((b: string, idx: number) => (
                        <li key={idx} className="flex items-start gap-3 text-base font-bold text-slate-800">
                  ...
                  >
                    <div className="bg-slate-50 border border-border p-8 rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.08)] relative overflow-hidden group hover:border-primary/50 transition-colors">
                      ...
                      {tech.type === 'software' && (
                        <div className="relative z-10">
                          <div className="flex items-center justify-between mb-6 pb-4 border-b border-border">
                            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Supported Interfaces</p>
                            <Settings className="w-5 h-5 text-slate-400" />
                          </div>
                          <div className="flex flex-col gap-3">
                            {tech.platforms.map((p: string, idx: number) => (
                              <div key={idx} className="px-4 py-3 bg-[#0A2647] hover:bg-primary text-white rounded-lg text-sm font-bold tracking-wide transition-colors cursor-default flex items-center gap-3">
                                <div className="w-2 h-2 bg-cyan-400 rounded-full" /> {p}
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {tech.type === 'protection' && (
                        <div className="relative z-10">
                           <div className="flex items-center gap-2 mb-6 border-b border-border pb-4">
                              ...
                              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Active Environmental Control</p>
                           </div>
                          <div className="flex flex-col gap-3">
                            {tech.protects.map((p: string, idx: number) => (
                              <div key={idx} className="px-4 py-2.5 bg-red-50 text-[#0A2647] border border-red-100 rounded-lg text-sm font-bold flex items-center gap-3 hover:bg-red-100 transition-colors">
                                <Wind className="w-4 h-4 text-primary" /> {p}
                              </div>
                            ))}
                          </div>
                        </div>
          ...
          <div className="container max-w-4xl">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-10 gap-4">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  onClick={copySpecs}
                  className="px-4 py-2 bg-white border border-border text-slate-700 font-bold text-xs uppercase tracking-wider rounded hover:bg-primary hover:text-white hover:border-primary transition-all shadow-sm flex items-center gap-2"
                >
        ...
        {/* 07. SYSTEM INTEGRATION (Specific to NanoStage) */}
        <section className="py-16 bg-slate-50/30 border-b border-border">
           <div className="container max-w-6xl">
             <div className="text-center max-w-3xl mx-auto mb-12">
               <span className="text-primary font-bold tracking-widest uppercase text-xs mb-2 block">Extensible Architecture</span>
               <h2 className="font-display text-3xl lg:text-4xl font-extrabold text-[#0A2647]">System Integration</h2>
             </div>
             
             <div className="grid md:grid-cols-3 gap-6">
                <motion.div whileHover={{ y: -5 }} className="bg-white rounded-2xl p-8 border border-border shadow-sm hover:shadow-md transition-all">
                  <h3 className="text-xl font-bold text-[#0A2647] mb-6 flex items-center gap-3">
                    <Activity className="w-5 h-5 text-primary" /> Mechanical Highlights
                  </h3>
                  <ul className="space-y-3">
                    {product.mechanicalHighlights.map((h: string, i: number) => (
                      <li key={i} className="flex gap-3 text-base text-slate-700 font-medium"><CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />{h}</li>
                    ))}
                  </ul>
                </motion.div>
                ...
                <motion.div whileHover={{ y: -5 }} className="bg-white rounded-2xl p-8 border border-border shadow-sm hover:shadow-md transition-all">
                  <h3 className="text-xl font-bold text-[#0A2647] mb-6 flex items-center gap-3">
                    <Settings className="w-5 h-5 text-primary" /> Optional Configurations
                  </h3>
                  <ul className="space-y-3">
                    {product.optionalConfigurations.map((opt: string, i: number) => (
                      <li key={i} className="flex gap-3 text-base text-slate-700 font-medium"><Settings className="w-5 h-5 text-blue-500 shrink-0 mt-0.5" />{opt}</li>
                    ))}
                  </ul>
                </motion.div>
                ...
                <motion.div whileHover={{ y: -5 }} className="bg-white rounded-2xl p-8 border border-border shadow-sm hover:shadow-md transition-all">
                  <h3 className="text-xl font-bold text-[#0A2647] mb-6 flex items-center gap-3">
                    <Layers className="w-5 h-5 text-primary" /> System Compatibility
                  </h3>
                  <ul className="space-y-3">
                    {product.systemCompatibility.map((sys: string, i: number) => (
                      <li key={i} className="flex gap-3 text-base text-slate-700 font-medium"><Layers className="w-5 h-5 text-purple-500 shrink-0 mt-0.5" />{sys}</li>
                    ))}
                  </ul>
                </motion.div>
             </div>
