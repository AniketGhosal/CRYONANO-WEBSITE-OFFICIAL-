...
import { 
  Download, ArrowUp, CheckCircle2, ChevronRight, ChevronDown, Plus, Minus,
  Activity, Zap, Shield, Camera, Cpu, Target, Settings, ThermometerSnowflake,
  Waves, Layers, Network, Lock, ShieldAlert, Code, ArrowLeft, ArrowRight, Move, Wind, Eye, Droplets, Orbit, ShieldCheck, Image, Copy
} from "lucide-react";
import { Link } from "react-router-dom";
import { productsData } from "@/data/products";
        ...
        return (
          ...
          >
            ...
            <div className="w-full md:w-1/4 flex flex-col items-center md:items-start text-center md:text-left shrink-0 border-b md:border-b-0 md:border-r border-slate-100 pb-6 md:pb-0 md:pr-6 relative z-10">
              ...
              <p className="text-xs font-extrabold text-slate-500 uppercase tracking-widest leading-normal group-hover:text-primary transition-colors">
                {tech.sideCaption}
              </p>
            </div>
            ...
            <div className="w-full md:w-3/4 flex flex-col justify-center relative z-10">
              ...
              {tech.type === 'software' && tech.platforms && (
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 block">Integrated Capabilities</p>
                  <div className="flex flex-wrap gap-2.5">
                    {tech.platforms.map((p: string, pIdx: number) => (
                      <div key={pIdx} className="px-4 py-2 bg-[#0A2647] text-white rounded-lg text-sm font-bold tracking-wide hover:bg-primary transition-colors cursor-default flex items-center gap-2">
              ...
              {tech.type === 'protection' && tech.protects && (
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 block">Safety & Protection Features</p>
                  <div className="flex flex-wrap gap-2.5">
                    {tech.protects.map((p: string, pIdx: number) => (
                      <div key={pIdx} className="px-4 py-2 bg-red-50 text-primary border border-red-100 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-red-100 transition-colors cursor-default">
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-background text-foreground antialiased selection:bg-primary selection:text-white relative">
        ...
        <div className="relative z-30 -mt-8 container max-w-6xl">
          <motion.div 
            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.6 }}
            className="bg-[#0A2647] rounded-xl border border-blue-900/50 p-6 shadow-2xl grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6"
          >
            {product.quickSpecs.map((spec: any, i: number) => (
              <div key={i} className="text-center border-r last:border-none border-blue-800/50 px-2 group">
        ...
        <div className="sticky top-[var(--nav-height)] z-40 w-full bg-white/95 backdrop-blur-md border-b border-border shadow-sm mt-12">
          <div className="container py-2 flex items-center justify-between">
            ...
            <div className="flex-1 flex justify-end">
              <Link to="/request-quote/research" className="hidden sm:inline-block text-center px-5 py-2 bg-primary hover:bg-red-600 text-white font-bold text-sm tracking-wider rounded transition-colors shadow-sm transform hover:-translate-y-0.5 uppercase">
                REQUEST A QUOTE
              </Link>
            </div>
          </div>
          ...
          <div className="container max-w-4xl">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-10 gap-4">
              ...
              <div className="flex gap-3">
                <button 
                  onClick={() => setOpenAccordion("all")} 
                  className="px-5 py-2 bg-white border border-border text-slate-700 font-bold text-sm uppercase tracking-wider rounded hover:bg-primary hover:text-white transition-colors shadow-sm"
                >
                  Expand All
                </button>
                <button 
                ...
                <button 
                  onClick={copySpecs}
                  className="px-5 py-2 bg-white border border-border text-slate-700 font-bold text-sm uppercase tracking-wider rounded hover:bg-primary hover:text-white transition-colors shadow-sm flex items-center gap-2"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
              {product.applications.map((app: string, i: number) => (
                <motion.div
                  ...
                  className="bg-slate-50 border border-border rounded-xl p-6 shadow-sm hover:shadow-lg hover:border-primary transition-all duration-300 flex flex-col items-center text-center"
                >
                  ...
                  <p className="text-sm text-slate-600">
                    High-uniformity magnetic field generation optimized for {app.toLowerCase()} environments.
                  </p>
                </motion.div>
              ))}
            </div>
