...
//               {/* REMOVED aspect-square, added h-auto and defined min-height layouts for stable image scaling parameters */}
...
//                         {/* Swapped h-full to h-auto and object-cover to object-contain, padding bottom to clear text captions safely */}
...
import { 
  ...
  Activity, Zap, Shield, Camera, Cpu, Target, Settings, ThermometerSnowflake,
  Waves, Layers, Lock, ShieldAlert, Code, ArrowLeft, ArrowRight, Move, Eye,
  Image, PackageSearch, Scaling, Thermometer, Droplets, Copy
} from "lucide-react";
import { Link } from "react-router-dom";
import { productsData } from "@/data/products";
...
// Icon mapping dictionary
const iconMap: Record<string, any> = { 
  ThermometerSnowflake, Activity, Zap, Shield, Camera, Move, Waves, Target, Cpu, Settings, Layers, Code, Eye, PackageSearch, Scaling, Thermometer, Droplets
};
        ...
        return (
          ...
          >
            ...
            <div className="w-full md:w-3/4 flex flex-col relative z-10">
              ...
              {tech.type === 'software' && tech.platforms && (
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 block">Ideal For</span>
                  <div className="flex flex-wrap gap-2">
                    {tech.platforms.map((p: string, i: number) => (
                      <div key={i} className="px-4 py-2 bg-[#0A2647] text-white rounded-md text-sm font-bold tracking-wide hover:bg-primary transition-colors cursor-default">
                        {p}
                      </div>
                    ))}
                  </div>
              ...
              {tech.type === 'protection' && tech.protects && (
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 block">Requirements Met</span>
                  <div className="flex flex-wrap gap-2">
                    {tech.protects.map((p: string, i: number) => (
                      <div key={i} className="px-4 py-2 bg-red-50 text-primary border border-red-100 rounded-md text-sm font-bold flex items-center gap-2 hover:bg-red-100 transition-colors cursor-default">
          ...
          <div className="container max-w-4xl">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-10 gap-4">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  onClick={copySpecs}
                  className="px-4 py-2 bg-white border border-border text-slate-700 font-bold text-xs uppercase tracking-wider rounded hover:bg-primary hover:text-white transition-colors shadow-sm flex items-center gap-2"
                >
              ...
              {product.applications.map((app: string, i: number) => (
                ...
                >
                  ...
                  <p className="text-base text-slate-600 leading-relaxed flex-1">
                    Ideal for advanced deployment and demanding technical procedures in {app.toLowerCase()} environments.
                  </p>
                </motion.div>
              ))}
            </div>
